...
func TestMatchPrefixString(t *testing.T) {
	tcases := []struct {
		a      key.Path
		b      key.Path
		result bool
	}{
		{
			...
			a:      key.Path{key.New("foo"), Wildcard, key.New("baz")},
			b:      key.Path{key.New("foo"), key.New("b")},
			result: true,
		}, {
			a:      key.Path{key.New("foo"), key.New("baz"), key.New("baz")},
			b:      key.Path{key.New("foo"), key.New("b")},
			result: true,
		}, {
			a:      key.Path{Wildcard, key.New("bar"), key.New("xyz")},
			b:      key.Path{key.New("foo"), key.New("b")},
			result: true,
		}, {
			...
			a:      key.Path{Wildcard, key.New(int32(0))},
			b:      key.Path{key.New("foo"), key.New("baz")},
			result: false,
		},
	}
	for i, tcase := range tcases {
		if result := MatchPrefixString(tcase.a, tcase.b); result != tcase.result {
			t.Fatalf("Test %d failed: a: %#v; b: %#v, result: %t",
				i, tcase.a, tcase.b, tcase.result)
		}
	}
